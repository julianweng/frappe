...
FMAP \citep{ganzfried2025consistent} defines the opponent model as the posterior mode in sequence form. In our implementation, we optionally add an entropy regularizer for numerical stability:
\begin{align}
\label{eq:fmap}
\max_{y \ge 0, \; Fy = f} \quad & \sum_{i=1}^{n} (\alpha_i - 1)\log y_i + \sum_{t=1}^{T} \log \Big( \sum_{j \in \mathcal{O}_t} q_{t,j} y_j \Big) \nonumber \\
& \quad - \lambda \sum_{i=1}^{n} y_i \log y_i,
\end{align}
where $\lambda \ge 0$ is an optional entropy weight. In implementation we minimize the \emph{negative} of \eqref{eq:fmap}, so the gradient used for optimization is
\begin{equation}
\label{eq:grad}
\nabla_i = \frac{1 - \alpha_i}{y_i} - \sum_{t: i \in \mathcal{O}_t} \frac{q_{t,i}}{\sum_{j \in \mathcal{O}_t} q_{t,j} y_j} + \lambda (1 + \log y_i),
\end{equation}
where the final term is the contribution of our added entropy regularizer. We discuss entropy regularization and discounted likelihood as optional extensions in Section~\ref{sec:safety}.
...
For theoretical clarity we define the \emph{minimization} form of the negative regularized log-posterior:
\[
F_T(y) \;=\; -\frac{1}{T}\sum_{t=1}^{T} \log \Big( \sum_{j \in \mathcal{O}_t} q_{t,j} y_j \Big)
\;-\;\frac{1}{T}\sum_{i=1}^{n} (\alpha_i - 1)\log y_i
\;+\;\lambda R(y),
\]
where $R(y) = \sum_i y_i \log y_i$ is the entropy regularizer. Up to an additive constant and positive scalar multiple, minimizing $F_T$ is equivalent to maximizing \eqref{eq:fmap}.
...
If $\lambda > 0$, then for each fixed $T$ the objective $F_T(y)$ is strictly convex on $\mathcal{D}_{\varepsilon}$ and therefore has a unique minimizer $\hat{y}_T$.
\end{proposition}
\begin{proof}
For each $t$, the map $y \mapsto \log\big(\sum_{j\in\mathcal{O}_t} q_{t,j}y_j\big)$ is concave on $\{y\ge 0\}$, hence its negation is convex on $\mathcal{D}_\varepsilon$. The log-prior term $y \mapsto \sum_i (\alpha_i-1)\log y_i$ is concave on $\{y_i>0\}$ when $\alpha_i\ge 1$, so its negation is convex on $\mathcal{D}_\varepsilon$. Finally, $R(y)=\sum_i y_i\log y_i$ is strictly convex on $\{y_i>0\}$, and restricting a strictly convex function to an affine slice preserves strict convexity. Thus $F_T$ is the sum of convex terms plus $\lambda R(y)$ with $\lambda>0$, hence strictly convex with a unique minimizer.
\end{proof}

\begin{proposition}[Algorithmic convergence of PGD and deterministic FW; expectation guarantees for stochastic variants]
\label{prop:alg-conv}
Let $\hat{y}_T = \arg\min_{y \in \mathcal{D}_\varepsilon} F_T(y)$ be the unique empirical minimizer for a fixed sample size $T$ (with $\lambda>0$).
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item There is no discounting ($\gamma=1$) and no entropy regularization ($\lambda=0$) in the FMAP objective.
\end{enumerate}
\end{assumption}
...
\begin{table}[ht]
\centering
\caption{Solver comparison (final preset, Kuhn poker).}
\label{tab:solver}
\begin{tabular}{lccc}
\toprule
...
Discounted likelihood ($\gamma=0.95$) improves recovery after opponent switches by emphasizing fresh observations, at the cost of a small steady-state bias. Entropy regularization ($\lambda>0$) reduces variance in stochastic gradients and empirically lowers oscillations in SFW at large batch sizes. Theoretically, these tools could further yield stable learning curves in the presence of non-stationary or adversarial opponents, but require further empirical study.
